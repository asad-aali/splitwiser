...
def load_data(args, task):
    ...
    if args.nproc == 1:
        ...
        datasets = []
        size_per_proc = int(len(list_finding) / args.nproc)
            ...
            data_list = [
                {
                    ...
                    "sentence": list_finding[i], 
                    "text_label": list_summary[i]
                } for i in range(size_per_proc*proc,(size_per_proc*proc)+size_per_proc,1)
            ]    
            datasets.append(Dataset.from_list(data_list))
        return datasets
...
def get_loader(dataset, tokenizer, batch_size, prompt_queue):
